...
$(document).ready(function () {
	...
	uagent = uagent.toString();
	
	var ck_ipad = uagent.split("ipad");
	var ck_android = uagent.split("android");
	var ck_iphone = uagent.split("iphone");
	var ck_blackberry = uagent.split("blackberry");
	var ck_webos = uagent.split("webos");
	
	*/


	ck_sizel(); //접속한 화면 크기 확인
	...
	$('body').on("managerManagement", function(event) {
		...
		if(winminsize==0){
			...
			alert("현재 기기의 해상도가 낮습니다.\n 감독 페이지를 원할하게\n 이용하시려면 해상도를 높여주세요.\n");
			$("#content").load("a-manager/manager_nominsize.html", function() {
				manager_onload();
			});
		}
